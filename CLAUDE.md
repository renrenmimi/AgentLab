...
4. **`/chance` 为什么每次不一样** ✅ — 采样与温度。同一个任务反复跑，计数条显示分布；
   温度 0 只走一条路，温度高时「不查就报数」那条也会出现。结论：agent 不是函数，
   「我试过能用」只是一次采样。模型在 `lib/chance.ts`，是纯函数。
5. **`/invent` 它为什么会编** ✅ — 幻觉，以及工具正是它的解药。三个问题（几点了 /
   用什么测试框架 / 一个不存在的函数）在「有工具 / 没工具」两种条件下对照。
   最关键的一条：工具还提供**事实的不存在**。
6. **`/instructions` 数组外面的那段话** ✅ — system 提示词的位置、每轮重发的账单
   （数字取自 `lib/cost.ts`）、以及「叮嘱一句」比「不给这个工具」弱一个量级。
   同一个任务三种条件：什么都没说 / 加一句 / 把 send_email 删掉。
8. **`/cost` 它为什么这么贵** ✅ — 每轮全量重发 ⇒ 花费随轮数平方增长。
   滑块拖轮数看曲线变弯；缓存开关看它变缓但依然是二次的。模型在 `lib/cost.ts`，
   是纯函数，verify.mjs 直接验它的结论（凸性、缓存在第 2 轮起划算）。
9. **`/context` 装不下的时候** ✅ — token 是什么、上下文窗口是什么、
   到顶之后的三条路（拒绝 / 截断 / 摘要）作用在同一段对话上，让人看见各自丢了什么。
7. **`/tools` 怎么描述工具** ✅ — description 是提示词不是文档。三个任务各暴露一种漏写：
   不做什么、工具集边界在哪、返回多大。
11. **`/trust` 工具结果不可信** ✅ — 提示注入。先演示（同一个网页的两种呈现）再命名，
   然后是缓解办法，每条都写清它挡不住什么。措辞类 vs 结构类的区分是这一站的重点。
12. **`/permission` 谁来说可以** ✅ — 把循环停在一次 write_file 前面，读者点「只这一次 /
   以后都允许 / 拒绝」，三条分支真的分岔。「以后都允许」那条会演示第三次写入
   在你不知情的情况下发生。边界画在副作用而不是风险上，判据是可逆性。
10. **`/delegate` 让另一个 agent 去做** ✅ — 一个想法：子 agent 换来上下文，
   代价是父 agent 再也无法核对那段摘要。
13. **`/again` 工具失败之后** ✅ — 三种失败（报错 / 超时 / 自己先断），只有第一种
    能确定动作没发生；退避表；幂等——搜索可重复、扣款不可，解法是幂等键。
    规矩：给 agent 的工具要么重复调用安全，要么带键。
14. **`/measure` 怎么知道变好了** ✅
15. **`/next` 这门课没讲的** ✅ — 四个方向（看真实运行 / 框架 / 认真做评测 / 多 agent），
    每个都写「为什么这门课没讲」。AgentTape 的指路移到了这里。 — 十个存好的任务 + 一个通过数；
